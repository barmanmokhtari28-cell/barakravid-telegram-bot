...
def fetch_recent_tweet_ids():
    ...
    # NOTE: r.jina.ai caches page renders by default, which was a major source of
    # delay (posts could show up minutes-to-hours late). "x-no-cache: true" forces
    # Jina to do a fresh live render instead of serving a stale cached snapshot.
    ...
    # NOTE: both the syndication endpoint and the free CORS proxies in front of it
    # cache responses at their edge. Appending a changing "_" query param busts
    # that caching so each run gets a fresh fetch instead of a stale cached one.
    cache_buster = int(time.time())
    target_syndication_url = (
        f"https://syndication.twitter.com/srv/timeline-profile/screen-name/"
        f"{TWITTER_TARGET_USER}?_={cache_buster}"
    )
    proxy_urls = [
        f"https://api.allorigins.win/get?url={target_syndication_url}&_={cache_buster}",
    ...
    for p_url in proxy_urls:
        try:
            ...
            resp = requests.get(p_url, headers=no_cache_headers, timeout=12)
            if resp.status_code == 200:
